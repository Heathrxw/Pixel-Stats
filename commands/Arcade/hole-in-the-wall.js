...
module.exports = {
    ...
    async execute(interaction) {;
        ...
        hypixelAPIReborn.getPlayer(username).then((player) => {
            const RecordFinals = (player.stats.arcade.holeInTheWall.scoreRecordFinals)
            const RecordNormal = (player.stats.arcade.holeInTheWall.scoreRecordNormal)
            const RecordOverall = (player.stats.arcade.holeInTheWall.scoreRecordOverall)
            ...
            con.query(`INSERT INTO HITW (Mode,Username,RecordFinals,RecordNormal,RecordOverall) VALUES ('HITW','${username}','${RecordFinals}','${RecordNormal}','${RecordOverall}')`)
        }).catch((err) => {
