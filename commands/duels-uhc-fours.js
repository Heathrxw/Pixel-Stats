...
module.exports = {
    ...
    async execute(interaction) {;
        ...
        hypixelAPIReborn.getPlayer(username).then((player) => {
            const Kills = (player.stats.duels.uhc.fours.kills)
            const Deaths = (player.stats.duels.uhc.fours.deaths)
            const Wins = (player.stats.duels.uhc.fours.wins)
            const Losses = (player.stats.duels.uhc.fours.losses)
            const KDR = (player.stats.duels.uhc.fours.KDRatio)
            const WLR = (player.stats.duels.uhc.fours.WLRatio)
            const playedGames = (player.stats.duels.uhc.fours.playedGames)
            const Winstreak = (player.stats.duels.uhc.fours.winstreak)
            const bestWinstreak = (player.stats.duels.uhc.fours.bestWinstreak)
            ...
            con.query(`INSERT INTO Duels (Mode,Username,Kills,Deaths,Wins,Losses,KDR,WLR,playedGames,Winstreak,bestWinstreak) VALUES ('UHCFours','${username}','${Kills}','${Deaths}','${Wins}','${Losses}','${KDR}','${WLR}','${playedGames}','${Winstreak}','${bestWinstreak}')`)
        }).catch((err) => {
