...
module.exports = {
    ...
    .setDescription('Gets specified players stats for UHC Doubles duels overall statistics')
    ...
    async execute(interaction) {;
        ...
        hypixelAPIReborn.getPlayer(username).then((player) => {
            const Kills = (player.stats.duels.uhc.doubles.kills)
            const Deaths = (player.stats.duels.uhc.doubles.deaths)
            const Wins = (player.stats.duels.uhc.doubles.wins)
            const Losses = (player.stats.duels.uhc.doubles.losses)
            const KDR = (player.stats.duels.uhc.doubles.KDRatio)
            const WLR = (player.stats.duels.uhc.doubles.WLRatio)
            const playedGames = (player.stats.duels.uhc.doubles.playedGames)
            const Winstreak = (player.stats.duels.uhc.doubles.winstreak)
            const bestWinstreak = (player.stats.duels.uhc.doubles.bestWinstreak)
            const doublesuhc = new MessageEmbed()
            .setColor(color)
            .setTitle(`${player}'s UHC Doubles Duels Statistics`)
            ...
            con.query(`INSERT INTO Duels (Mode,Username,Kills,Deaths,Wins,Losses,KDR,WLR,playedGames,Winstreak,bestWinstreak) VALUES ('UHCDoubles','${username}','${Kills}','${Deaths}','${Wins}','${Losses}','${KDR}','${WLR}','${playedGames}','${Winstreak}','${bestWinstreak}')`)
        }).catch((err) => {
